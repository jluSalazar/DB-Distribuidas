...
namespace WinAppBiblioteca.Forms
{
    public partial class EmpleadoActualizar : Form
    {
        ...
        Usuario user;
        string conStr;
        public EmpleadoActualizar(Usuario usuario)
        {
            InitializeComponent();
            user = usuario;
        ...
        private void iconButton1_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txt_ID.Text))
            {
                ...
                if (user.IsMaster)
                {
                    deleteQuery = "DELETE FROM empleado WHERE IDEMPLEADO = :idEmpleado";
                }
                else
                {
                    deleteQuery = "DELETE FROM empleado@replica_proyrad WHERE IDEMPLEADO = :idEmpleado";
                }
                ...
                deleteCommand.Parameters.Add(":idEmpleado", OracleDbType.Varchar2).Value = id;
